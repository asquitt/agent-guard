...
- [x] API documentation:
  - docs/api-reference.md: full endpoint table, auth, error formats, rate limits
  - OpenAPI auto-generated by FastAPI at /docs and /redoc
- [x] Integration guides:
  - docs/quickstart.md: 5-minute setup (sign up → install → 3 lines of code)
  - docs/integration-guide.md: Python, Node.js, curl, streaming, blocked responses
- [x] Lightweight SDK (Python):
  - sdk/python/ with pyproject.toml (hatchling build)
  - `agentguard.wrap_openai(client)` and `agentguard.wrap_anthropic(client)`
  - `AgentGuardClient` for direct API access (incidents, detectors)
