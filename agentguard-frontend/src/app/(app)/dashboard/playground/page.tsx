  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <span className="text-xs font-medium text-muted-foreground self-center">Quick tests:</span>
        {SAMPLE_PAYLOADS.map((s, i) => (
          <button
            key={s.label}
            onClick={() => loadSample(i)}
            className="rounded-full border border-border bg-card px-3 py-1 text-xs font-medium text-foreground hover:border-primary/30 hover:text-primary"
          >
            {s.label}
          </button>
        ))}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white hover:bg-primary/100 disabled:opacity-50"
          >
        ...
        {/* Results panel */}
        <div>
          <h3 className="mb-3 text-sm font-medium text-foreground">Results</h3>
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {hit.detected && (
              <span
                className={clsx(
                  'inline-flex rounded-full px-2 py-0.5 text-xs font-medium',
                  SEVERITY_COLORS[hit.severity] ?? 'bg-muted text-muted-foreground',
                )}
              >
                {hit.severity}
              </span>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{hit.title}</p>
          {hit.description && (
      ...
      {expanded && (
        <pre className="mt-3 max-h-48 overflow-auto rounded bg-card p-3 text-xs text-foreground">
