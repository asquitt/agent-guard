...
description: Run the required read-only exact-base-to-head senior review for material AgentGuard changes before merge or deployment.
---

# AgentGuard Independent Review

Use this only after the implementation and focused verification are complete and the candidate is committed.

1. Record full immutable base and head SHAs. When a pull request exists, also record its `potentialMergeCommit.oid`. Do not review an uncommitted worktree or moving branch name.
2. Give one separate xhigh reviewer the user requirements, architecture affected, tests, runtime context, base, head, merge-candidate identity when applicable, and `.github/ai-review/senior-review.md`.
3. The reviewer stays read-only, inspects `git diff BASE..HEAD`, the named merge candidate when supplied, and directly affected call paths, then challenges the claimed property with one focused adversarial or real-runtime probe when practical.
